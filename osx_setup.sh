...
# A stripped down version of the gist by brandonb927
...
echo ""
echo "Where do you want screenshots to be stored? (hit ENTER if you want ~/Downloads as default)"
# Thanks https://github.com/omgmog
read screenshot_location
echo ""
if [ -z "${screenshot_location}" ]
then
  # If nothing specified, we default to ~/Downloads
...
case $response in
  [yY])
    brew install python
    echo ""
    echo "Would you like to install deliverables from the pip requirements.txt ? (y/n)"
    read -r response
    case $response in
      [yY])
...
echo ""
echo "Would you like to install ffmpeg ? (y/n)"
read -r response
case $response in
  [yY])
    brew install ffmpeg
